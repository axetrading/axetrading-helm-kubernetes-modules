...
resource "aws_iam_policy" "thanos_s3_access" {
  ...
  name_prefix = "thanos-s3-bucket-policy-"
  path        = var.role_path
  description = "Policy for Thanos Sidecar to write to S3 bucket"

  policy = data.aws_iam_policy_document.s3_access[0].json
}

resource "aws_iam_role_policy_attachment" "thanos_s3_access_attachment" {
  count = var.create_role && var.enabled ? 1 : 0

  role       = aws_iam_role.this[0].name
  policy_arn = aws_iam_policy.thanos_s3_access[0].arn
}
