...
{{/*
Set of validation rules for Postgres Exporter Datasources
*/}}
{{- define "prometheus-postgres-exporter.validateValues" }}
{{- if not (or .Values.config.datasource .Values.config.datasourceSecret) }}
  {{- fail "ERROR: You must provide a value for either .Values.config.datasource or .Values.config.datasourceSecret" }}
{{- end }}
{{- if and .Values.config.datasource.passwordSecret .Values.config.datasource.password }}
  {{- fail "ERROR: only one of .Values.config.datasource.passwordSecret and .Values.config.datasource.password must be defined" }}
{{- end }}
{{- end }}
