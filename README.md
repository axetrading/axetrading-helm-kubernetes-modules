...
| <a name="input_monitored_endpoints"></a> [monitored\_endpoints](#input\_monitored\_endpoints) | The endpoints to be monitored by Prometheus | <pre>object({<br>    http_endpoints = optional(list, [])<br>    tcp_endpoints  = optional(list, [])<br>    icmp_endpoints = optional(list, [])<br>    ssh_endpoints  = optional(list, [])<br>  })</pre> | <pre>{<br>  "http_endpoints": [],<br>  "icmp_endpoints": [],<br>  "ssh_endpoints": [],<br>  "tcp_endpoints": []<br>}</pre> | no |
